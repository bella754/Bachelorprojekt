<!-- <script setup>
    ...
    const userID = ref(route.query.userID);  // userID aus der URL-Query auslesen
    //console.log("userID in formular: ", userID);
...
Object.keys(schemaProperties).forEach(key => {
    formData.value[key] = '';         
    fieldErrors.value[key] = false; // Keine Fehler anfangs
});
...
// Verhindert nicht-numerische Eingaben für Integer-Felder
function preventNonInteger(event) {
    ...
    <div class="form_container">
        ...
        <form class="form">
            <div v-for="key in requiredFields" :key="key">
                ...
                <select v-if="schemaProperties[key].enum" class="form_input" :class="{'input_error': fieldErrors[key]}" :id="key" v-model="formData[key]">
                    <option v-for="option in schemaProperties[key].enum" :key="option" :value="option">{{ option }}</option>
                </select>

                <textarea v-else-if="schemaProperties[key].type === 'textarea'" class="form_input form_input_textarea" :class="{'input_error': fieldErrors[key]}" :id="key" v-model="formData[key]" :placeholder="'Bitte ' + key + ' angeben'" />

                <input v-else-if="schemaProperties[key].type === 'integer'" step="1" min="0" placeholder="Nur ganze Zahlen" class="form_input" :class="{'input_error': fieldErrors[key]}" :id="key" v-model="formData[key]" type="number" @keydown="preventNonInteger"/>

                <input v-else-if="schemaProperties[key].type === 'string'" class="form_input" :class="{'input_error': fieldErrors[key]}" :id="key" v-model="formData[key]" type="text" :placeholder="'Bitte ' + key + ' angeben'" />

                <input v-else-if="schemaProperties[key].type === 'number'" class="form_input" :class="{'input_error': fieldErrors[key]}" :id="key" v-model="formData[key]" type="number" />

                <input v-else-if="schemaProperties[key].type === 'date'" class="form_input" :class="{'input_error': fieldErrors[key]}" :id="key" v-model="formData[key]" type="date" />

                <!-- Fehleranzeige unter jedem Feld -->
                <p v-if="fieldErrors[key]" class="error_message">{{ fieldErrors[key] }}</p>
            </div>
        </form>
        <button class="form_submit" @click="submit_form()">Erstellen</button>
    </div>
...
/* Fehlerhafte Eingaben rot markieren */
.input_error {
